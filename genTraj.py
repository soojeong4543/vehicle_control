...
def genTrajectory(options, scene_const, next_veh_pos, next_veh_heading, next_state_sensor, next_state_goal,
                  network_model, max_horizon):
    # Set curr variable
    ...
    for t in range(0, max_horizon):
        ...
        targetSteer_k = scene_const.max_steer - action_stack_k * abs(scene_const.max_steer - scene_const.min_steer) / (
        options.ACTION_DIM - 1)
        ...
        newGoal = getGoalEstimation(options, scene_const, newPosition, newHeading,
                                    oldGoal[:, :, options.FRAME_COUNT - 1])
...
def getGoalEstimation(options, scene_const, veh_pos, veh_heading, g_pos):
    ...
    for k in range(0, options.VEH_COUNT):
        ...
        gInfo[k][0] = math.atan(
            abs(delta_distance[0]) / abs(delta_distance[1]))  # delta x / delta y, and scale by pi 1(left) to -1 (right)
    ...
    for v in range(0, options.VEH_COUNT):
        ...
        for i in range(0, scene_const.sensor_count):
            for j in range(0, length - 1):
                ...
                A = np.array([[newTransXY[i][1], -newTransXY[i][0]], [reducedLidarXY[l][1] - reducedLidarXY[j][1],
                                                                      -reducedLidarXY[l][0] + reducedLidarXY[j][0]]])
                ...
                try:
                    ...
                    if np.dot(A[0, :], reducedLidarXY[j] - x) * np.dot(A[0, :], reducedLidarXY[l] - x) <= 0:
                        newLidarXY[i, :] = x
                        newSensorState[v, i] = oldSensorState[v, j] * oldSensorState[
                            v, l]  # closed if there exist at least closed point
                        flag[i] = 0
                except:
                    pass
        ...
        newLidarXY = newLidarXY + 1.5 * scene_const.collision_distance * np.transpose(
            [np.cos(seqAngle), np.sin(seqAngle)]) * np.transpose([flag, flag])
...
sample_state_sensor = np.ones(
    (sample_options.VEH_COUNT, sample_scene_const.sensor_count * 2, sample_options.FRAME_COUNT))
sample_state_goal = np.ones((sample_options.VEH_COUNT, 2, sample_options.FRAME_COUNT)) * 100
max_horizon = 5

traj_est = genTrajectory(sample_options, sample_scene_const, sample_veh_pos, sample_veh_heading, sample_state_sensor,
                         sample_state_goal, network_model, max_horizon)