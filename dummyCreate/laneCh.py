...
for x in np.arange(0,60,0.5):
    if x <  20:
        ...
        _ = vrep.simxSetObjectPosition(clientID, dhandle, -1, [x, 7.5*0.25/25*(((x-20)/0.5)**2), 0],
        ...
        _ = vrep.simxSetObjectPosition(clientID, dhandle, -1, [x, -7.5 * 0.25 / 25 * (((x - 40) / 0.5)** 2)+15, 0],vrep.simx_opmode_oneshot)
        ...
        _ = vrep.simxSetObjectPosition(clientID, dhandle, -1, [x, 7.5 * 0.25 / 25 * (((-x + 50) / 0.5) ** 2), 0],vrep.simx_opmode_oneshot)
    else :
        _, dhandle = vrep.simxCreateDummy(clientID, 0.3, [255,0,0], vrep.simx_opmode_blocking)
        _ = vrep.simxSetObjectParent(clientID, dhandle, dsethandle, True, vrep.simx_opmode_blocking)
...
for index in range(0,len(refHandle)-1):
    ...
    _ = vrep.simxSetObjectPosition(clientI,refHandle[index],-1,[curPos[0],curPos[1],0.1],vrep.simx_opmode_blocking)
